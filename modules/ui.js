...
export function renderResultado(r) {
  ...
  if (vendedorEl) vendedorEl.textContent = r._meta.vendedor;
      ...
      ${r.fases.map(f => {
        ...
        return `
          <div class="fase-item">
            ...
            <div class="fase-text">${f.texto}</div>
          </div>`;
      }).join('')}
    </div>`;
  ...
  if (r.objeciones?.length > 0) {
    ...
    container.appendChild(makeBlock('⚡ OBJECIONES DETECTADAS',
      '<p class="rb-text" style="color:var(--text-dim);">No se detectaron objeciones explícitas.</p>', true));
  }
  ...
  const spHTML = `
    <div class="paso-grid">
      ...
      <div class="paso-item">
        ...
        <div class="paso-value">${sp.accion_concreta}</div>
      </div>
    </div>
  ...
  // 4. Patrón de error
  container.appendChild(makeBlock('⚠ PATRÓN DE ERROR PRINCIPAL',
    `<div class="error-box"><p>${r.patron_error}</p></div>`, true));
