// v21 — strict top-level block matching for objecion/siguiente_accion
...
async function processJobAsync(jobId: string) {
  ...
  try {
    ...
    console.log(`[analyze v21] Parsed ${parsed.phases.length} phases, matched ${phasesWithIds.length}, phase_ids: ${JSON.stringify(phasesWithIds.map(p => p.phase_id))}`);
