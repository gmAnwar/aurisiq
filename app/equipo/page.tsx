  ...
  useEffect(() => {
    async function load() {
      ...
      const [teamRes, weekRes, prevRes, todayRes, yesterdayRes, objRes, alertsRes, descalRes, recentRes, leadSourcesRes, stagesRes] = await Promise.all([
        ...
        supabase.from("analyses").select("user_id, score_general, categoria_descalificacion, lead_quality")
      ...
      setTodayQualified(today.filter(a => (a as { lead_quality?: string }).lead_quality === "calificado").length);
